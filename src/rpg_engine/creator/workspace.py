...
class CreatorWorkspace:
    ...
    def _resource_path(self, kind: str, resource_id: str) -> Path:
        ...
        safe_id = self._validate_id(resource_id)
        return self.root / spec.directory / f"{safe_id}.yaml"
    ...
    def initialize(
        ...
        *,
        ...
        with self._lock:
            ...
            mod = ModMetadata(engine=engine)
            self._atomic_write_yaml(self.manifest_path, _json_payload(manifest))
            self._atomic_write_yaml(self.mod_path, _json_payload(mod))
    ...
    def put(self, kind: str, resource_id: str, payload: dict[str, Any]) -> ResourceRecord:
        ...
        with self._lock:
            self._atomic_write_yaml(path, dumped)
    ...
    def load_registry(self) -> ContentRegistry:
        """Load the workspace through the same authoritative content loader used by the engine."""
    ...
    def map_graph(self) -> MapGraph:
        ...
        locations = {record.id for record in self.list_resources("location")}
        connections = self.list_resources("connection")
        nodes: list[MapNode] = []
        for resource_id in sorted(locations):
            payload = self.get("location", resource_id).payload or {}
            point = layout.positions.get(resource_id)
            nodes.append(
                MapNode(
                    id=resource_id,
                    name=str(payload.get("name", resource_id)),
